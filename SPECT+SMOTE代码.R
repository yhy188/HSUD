library(readxl)
SPECT <- read_excel("C:/Users/11988/Desktop/Êý¾Ý/SPECT.xlsx")
View(SPECT)
dim(SPECT)

library(DMwR)
SPECT$y<-as.factor(SPECT$y)
data1<-SMOTE(y~.,as.data.frame(SPECT),perc.over=860,perc.under=960)
...
dim(data)
set.seed(1)
train_obs = sample(nrow(data), 386)
...
SL.caret
set.seed(1)
sl_lasso = SuperLearner(Y = y_train, X = x_train, family = binomial(),cvControl = list(V = 10),

library(readxl)
framingham <- read_excel("C:/Users/11988/Desktop/framingham.xlsx")
View(framingham)
dim(framingham)

library(DMwR)
framingham$y<-as.factor(framingham$y)
data1<-SMOTE(y~.,as.data.frame(framingham),perc.over=860,perc.under=960)
...
dim(data)
set.seed(1)
train_obs = sample(nrow(data), 16577)
